...
HOTSPOT_PSM=`fast_psmcli get dmsb.hotspot.enable`
...
case $SELFHEAL_TYPE in
    "BASE"|"SYSTEMD")
        if [ "$BOX_TYPE" != "HUB4" ] && [ "$BOX_TYPE" != "SR300" ] && [ "$BOX_TYPE" != "SE501" ] && [ "$thisIS_BCI" != "yes" ] && [ $BR_MODE -eq 0 ] && [ ! -f "$brlan1_firewall" ] && [ "$FIRMWARE_TYPE" != "OFW" ]; then
...
#       esac
        ...
        case $SELFHEAL_TYPE in
            "BASE"|"SYSTEMD")
                if [ "$thisIS_BCI" != "yes" ] && [ "$brlan1up" = "" ] && [ "$BOX_TYPE" != "HUB4" ] && [ "$BOX_TYPE" != "SR300" ] && [ "$BOX_TYPE" != "SE501" ] && [ "$FIRMWARE_TYPE" != "OFW" ]; then
...
isPeriodicFWCheckEnable=$(get_from_syscfg_cache PeriodicFWCheck_Enable)
